...
local attention = require 'visu.attention_visu'
...
function layer:sample(inputs, opt)
  ...
  for t=1,self.seq_length+1 do
    ...
    logprobs, attenprobs = unpack(self.attention:forward(atten_input))
  end
...
function layer:sample_beam(inputs, opt)
  ...
  local atten = torch.FloatTensor(self.seq_length, batch_size, 50)
  ...
  for k=1,batch_size do
    ...
    local logprobs, attenprobs -- logprobs predicted in last time step, shape (beam_size, vocab_size+1)
    local done_beams = {}
    ...
    local attention = torch.FloatTensor(self.seq_length, beam_size, 50):zero()

    for t=1,self.seq_length+1 do

      local xt, it, sampleLogprobs
      local new_state
      local new_atten = torch.Tensor(beam_size,50):zero()
      if t == 1 then
      ...
      if t > 1 then
        attention:narrow(1,t-1,1):copy(new_atten)
      end
      ...
      logprobs, attenprobs = unpack(self.attention:forward(atten_input))
      attenprobs = attenprobs:view(beam_size, 50):float()
    end
    ...
    atten[{{}, k, {}}] = attention[{{},{done_beams[1].idx},{}}]
  end
...
function layer:updateOutput(input)
  ...
  self:_createInitState(batch_size)
  local atten = torch.FloatTensor(self.seq_length+1, batch_size, 50)
  ...
  for t = 1,self.seq_length+1 do
    ...
    if not can_skip then
      ...
      local atten_out, attenprobs = unpack(self.attention:forward(self.atten_inputs[t]))

      atten:narrow(1,t,1):copy(attenprobs:view(batch_size, 50))
...
function crit:updateOutput(inputs)
  ...
  --[[
  -- making the seq with the end token. 
  local eseq = eseq or torch.CudaTensor()
  eseq:resize(L, N):zero()
  eseq:narrow(1,1,D):copy(seq) -- copy the seq

  eseq:scatter(1,seq_len:add(1):view(1,-1), Mp1)            -- insert the END token
  eseq = eseq:view(-1,1)
  -- making a mask by using the seq
  local mask = mask or torch.CudaByteTensor()
  mask:resize(eseq:size()):zero()
  mask[torch.eq(eseq, 0)] = 1

  local input_reshape = input:view(-1, Mp1):clone()
  self.gradInput:resizeAs(input_reshape):zero() -- reset to zeros

  local loss_vec = input_reshape:gather(2, eseq):maskedFill(mask,0)
  local n = mask:size(1) - torch.sum(mask)
  self.output = -torch.sum(loss_vec) / n

  self.gradInput:scatter(2,eseq,-1):maskedFill(mask:expandAs(self.gradInput),0):div(n)
